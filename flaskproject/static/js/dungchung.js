...
function setupEventTaiKhoan() {
  ...
  // =================================================================
  // DÒNG SỬA LỖI ĐÂY:
  // Thêm kiểm tra, nếu không tìm thấy phần tử 'taikhoan' thì thoát hàm
  if (!taikhoan) {
    return;
  }
  // =================================================================

  var list = taikhoan.getElementsByTagName("input"); // Tạo eventlistener cho input để tạo hiệu ứng label // Gồm 2 event onblur, onfocus được áp dụng cho từng input trong list bên trên
  ...
  for (var i = 0; i < tab.length; i++) {
    ...
    a.addEventListener("click", function (e) {
      e.preventDefault(); // tắt event mặc định // Thêm active(màu xanh lá) cho li chứa tag a này => ấn login thì login xanh, signup thì signup sẽ xanh

      this.parentElement.classList.add("active"); // Sau khi active login thì phải tắt active sigup và ngược lại // Trường hợp a này thuộc login => <li>Login</li> sẽ có nextElement là <li>SignUp</li>
      ...
      } // Ẩn phần nhập của login nếu ấn signup và ngược lại // href của 2 tab signup và login là #signup và #login -> tiện cho việc getElement dưới đây
  ...
  } // Đoạn code tạo event trên được chuyển về js thuần từ code jquery // Code jquery cho phần tài khoản được lưu ở cuối file này
}
...
function capNhat_ThongTin_CurrentUser() {
  ...
  // DÒNG SỬA LỖI: Lấy các phần tử HTML trước
  var cartNumberEl = document.getElementsByClassName("cart-number")[0];
  var memberEl = document.getElementsByClassName("member")[0];
  var menuMemberEl = document.getElementsByClassName("menuMember")[0];

  // Chỉ chạy code nếu user đã đăng nhập VÀ tất cả các phần tử HTML đều tồn tại
  if (u && cartNumberEl && memberEl && menuMemberEl) {
    // Cập nhật số lượng hàng vào header
    cartNumberEl.innerHTML = getTongSoLuongSanPhamTrongGioHang(u); // Cập nhật tên người dùng
    memberEl.getElementsByTagName("a")[0].childNodes[2].nodeValue =
      " " + u.username; // bỏ class hide của menu người dùng
...
function autocomplete(inp, arr) {
  if (!inp) {
    return; // Nếu không có ô input (inp) thì thoát hàm
  }
  var currentFocus;
...
function addTags(nameTag, link) {
  var new_tag = `<a href=` + link + `>` + nameTag + `</a>`; // Thêm <a> vừa tạo vào khung tìm kiếm

  var khung_tags = document.getElementsByClassName("tags")[0];

  // ============================================
  // THÊM KIỂM TRA NÀY VÀO
  if (!khung_tags) {
    return; // Nếu không tìm thấy 'khung_tags', thì thoát
  }
  // ============================================
